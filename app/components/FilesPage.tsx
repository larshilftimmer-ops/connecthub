...
.eq("shared_with", user.email);
 ...
 shared_with_me: true,
 created_at: share.created_at || undefined
 });
 }
 }
...
...file,
 ...
 setMessage({ type: 'error', text: 'Dateityp nicht erlaubt. Nur PDF, Bilder, Word, Excel.' });
...
.upload(filePath, selectedFile, {
 ...
 if (!confirm(`Datei "${file.name}" wirklich unwiderruflich löschen?\n\nDSGVO Art. 17: Diese Aktion kann nicht rückgängig gemacht werden.`)) return;
...
.getPublicUrl(file.path);
 ...
 accept=".pdf,.jpg,.jpeg,.png,.gif,.webp,.doc,.docx,.txt,.xls,.xlsx"
 className="hidden"
 />
 <button
...
? "bg-[#00D9FF]/20 text-[#00D9FF] border border-[#00D9FF]/50"
 ...
 <span className="font-semibold text-white">DSGVO:</span> Dateien verschlüsselt gespeichert. Nur eingeladene Personen haben Zugriff. Löschung jederzeit möglich (Art. 17).
 </p>
 </div>
 </div>
 ...
 <strong>DSGVO Art. 13 & 32:</strong><br/>
 - Datei wird verschlüsselt auf Servern in Deutschland gespeichert<br/>
 - Nur du und explizit eingeladene Personen haben Zugriff<br/>
 - Du kannst die Datei jederzeit löschen (Art. 17)<br/>
 - Keine Weitergabe an Dritte ohne deine Zustimmung
 </p>
 </div>
 </div>
